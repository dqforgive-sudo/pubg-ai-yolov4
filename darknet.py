...
def load_network(config_file, data_file, weights, batch_size=1):
    ...
    metadata = load_meta(data_file.encode("UTF-8"))
...
def draw_boxes(detections, image, colors):
    ...
    for label, confidence, bbox in detections:
        ...
        # 在绘制文本之前，将OpenCV图像转换为Pillow图像
        pil_image = Image.fromarray(cv2.cvtColor(image, cv2.COLOR_BGR2RGB))
        draw = ImageDraw.Draw(pil_image)

        # 设置字体和字体大小
        font = ImageFont.truetype('lib/simsun.ttc', size=20)

        # 设置中文文本内容和位置
        chinese_text = "{} [{:.2f}%]".format(label, float(confidence))
        text_position = (left, top - 25)

        # 在图像上绘制中文文本
        draw.text(text_position, chinese_text, font=font, fill=colors[label])
...
if os.name == "posix":
    ...
    lib = ct.CDLL("darknet.dll", mode = ct.RTLD_GLOBAL)
else:
